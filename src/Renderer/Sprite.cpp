...
namespace Renderer {
	Sprite::Sprite(const std::shared_ptr<Texture2D>& pTexture,
		...
		const glm::vec2& size,
		const float& rotation) : m_pTexture(pTexture), m_pShaderProgram(pShaderProgram), 
	...
	void Sprite::setPosition(const glm::vec2& position) { m_position = position; }

	void Sprite::setSize(const glm::vec2& size) { m_size = size; }
